...
class Controller(port: Int) {
    ...
    val btnBack = createButton(XboxController.Button.kBack.value)
    val btnStart = createButton(XboxController.Button.kStart.value)
    ...
    val joystickClickL = createButton(XboxController.Button.kStickLeft.value)
    val joystickClickR = createButton(XboxController.Button.kStickRight.value)

    val dpadUp = Button { getDpad(315, 0, 45) }
    val dpadRight = Button { getDpad(45, 90, 135) }
    val dpadDown = Button { getDpad(135, 180, 225) }
    val dpadLeft = Button { getDpad(225, 270, 315) }
    ...
    private fun getDpad(a: Int, b: Int, c: Int): Boolean {
        val pov: Int = controller.getPOV()
        return (pov == a) or (pov == b) or (pov == c)
    }
