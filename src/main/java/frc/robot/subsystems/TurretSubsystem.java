...
public class TurretSubsystem extends SubsystemBase {
  ...
  public void setGoalAngle(double newGoalAngle) {
    if (!isAngleInBounds(newGoalAngle)) {
      logger.error("Something tried to set the turret out of bounds! Next time, use isAngleInBounds() first.");
      return;
    }
  ...
  public boolean atGoalAngle() {
    return Math.abs(goalAngle - getCurrentAngle()) < slowDownWithinThisAngleFromGoal * minimumPercentOutput;
  }

  public double getCurrentAngle() {
    double tentativeCurrentAngle = startAngle + motor.getOutputRevsCompleted() * 360.0;
    if(tentativeCurrentAngle < minAngle - slowDownWithinThisAngleFromGoal || tentativeCurrentAngle > maxAngle + slowDownWithinThisAngleFromGoal) {
